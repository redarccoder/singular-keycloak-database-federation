...
public class DBUserStorageProvider implements UserStorageProvider,
    ...
    DBUserStorageProvider(KeycloakSession session, ComponentModel model, DataSourceProvider dataSourceProvider,
            QueryConfigurations queryConfigurations, UserConfiguration userConfiguration) {
    ...
    public boolean isValid(RealmModel realm, UserModel user, CredentialInput input) {
        ...
        if (allowDatabaseToOverwriteKeycloak && user instanceof CachedUserModel
    ...
    public UserModel getUserById(RealmModel realm, String id) {
        ...
        if (user == null) {
            ...
            return new UserAdapter(session, realm, model, user, allowDatabaseToOverwriteKeycloak, userConfiguration);
        }
    }
    ...
    public Stream<UserModel> searchForUserStream(RealmModel realm, Map<String, String> params, Integer firstResult,
        ...
        return internalSearchForUser(params.values().stream()
                .filter(s -> s.contains("keycloak.session.realm.users.query.search")).findFirst().orElse(null), realm,
                null);
    }
    ...
    public boolean removeUser(RealmModel realm, UserModel user) {
        ...
        if (userRemoved) {
            log.infov("deleted keycloak user: realm={0} userId={1} username={2}", realm.getId(), user.getId(),
                    user.getUsername());
        }
